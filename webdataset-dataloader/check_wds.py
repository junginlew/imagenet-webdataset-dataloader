...
def test_pipeline():
    ...
    ax.plot(x, batch_throughputs, color="steelblue", linewidth=1, alpha=0.6, label="배치별 처리량")
    ax.axhline(throughput, color="tomato", linewidth=1.5, linestyle="--", label=f"평균 {throughput:.1f} img/s")
    ax.set_xlabel("배치 번호")
    ax.set_ylabel("처리량 (img/s)")
    ax.set_title("S3 스트리밍 처리량 (WDS)")
    ax.legend()
